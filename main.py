...
def main():
    ...
    serial = Serial("/dev/ttyUSB0", 921600, timeout=0.1)
    
    # Set MIT Mode.
    mode = 2
    mode_data = f"FF FF FF FF FF FF FF {mode:02X}"
    mode_serial = can2serial("00000001", mode_data)
    serial.write(mode_serial)
    time.sleep(0.05)
    raw_data = serial.read(serial.in_waiting)
    print(f"Sent MIT Mode command via serial. Motor is ready.")
    
    # Send motor command: move to position 0.0 with kp=20, kd=0.5
    position = 0.5
    velocity = 0.0
    kp = 20.0
    kd = 0.5
    ...
    print(f"Sending command: position={position}, kp={kp}, kd={kd}")
    ...
    raw_data = serial.read(serial.in_waiting)
    if raw_data:
        print(f"Raw response: {raw_data.hex()}")
        can_data = raw_data[7:-2]
        if can_data and len(can_data) >= 8:
            position, velocity, torque, vbus, temp = mit_func.decode_reply(can_data)
            print(f"Motor State: p={position:.3f}, v={velocity:.3f}, torque={torque:.3f}, vbus={vbus:.3f}, temp={temp:.3f}")
